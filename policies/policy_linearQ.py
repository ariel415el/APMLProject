...
class LinearQ(bp.Policy):
    ...
    def init_run(self):
        ...
        self.b_periodic = np.random.normal(size=(num_actions,))
    ...
    def learn(self, round, prev_state, prev_action, reward, new_state, too_slow):
        ...
        if batch_size > 0:
            ...
            w_gradient = np.zeros(self.W_trainable.shape)
            b_gradient = np.zeros(self.b_trainable.shape)
            for j in range(batch_size):
                future_q_val = np.max(np.dot(self.W_periodic, batch_next_states[j]) + self.b_periodic)
                last_round_trainable_out = np.dot(self.W_trainable, batch_prev_states[j]) + self.b_trainable
                delta = last_round_trainable_out[batch_prev_actions[j]] - batch_rewards[j] - DISCOUNT*future_q_val

                w_gradient[batch_prev_actions[j]] += (LR*delta/batch_size)*batch_prev_states[j]
                b_gradient[batch_prev_actions[j]] += (LR*delta/batch_size)

            self.W_trainable -= LR*w_gradient
            self.b_trainable -= LR*b_gradient
            gs_num += 1

            if gs_num % UPDATE_STEPS == 0:
                self.W_periodic = self.W_trainable
                self.b_periodic = self.b_trainable
    ...
    def act(self, round, prev_state, prev_action, reward, new_state, too_slow):
        ...
        if np.random.rand() < self.epsilon:
            ...
            net_out = np.dot(self.W_periodic, new_state_vec) + self.b_periodic
